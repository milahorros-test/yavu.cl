...
;	});
	...
	function CanjearTicket()
	{
		...
		var route = "http://yavu.hol.es/public/canjearticket/"+user_id;
		$.ajax({
			url: route,
	...
	function UsarTicket(sorteo_id)//Esto deberia insertar un ticket en negativo y dejarlo rendido para el sorteo correspondiente.
	{
		...
		var route = "http://yavu.hol.es/public/usarticket/"+user_id+"/"+sorteo_id;
		$.ajax({
			url: route,
			headers: {'X-CSRF-TOKEN': token},
			type: 'GET',
	...
	{
		var user_id = $("#user_id").val();
		var route = "http://yavu.hol.es/public/verificartickets/"+user_id;
